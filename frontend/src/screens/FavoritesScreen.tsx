...
export default function FavoritesScreen() {
  ...
  const renderItem = ({ item, index }: { item: FavoriteItem; index: number }) => (
    <View style={{ padding: 12, backgroundColor: "#1a1a1a", borderRadius: 12, marginBottom: 12, flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
      <View style={{ flex: 1 }}>
      ...
      <TouchableOpacity onPress={() => handleRemoveFavorite(index)} style={{ marginLeft: 12, padding: 6, backgroundColor: "#ff6b6b", borderRadius: 6 }}>
  ...
  return (
    <View style={{ flex: 1, padding: 20, backgroundColor: "#000" }}>
      <Text style={{ fontSize: 22, fontWeight: "600", marginTop: 25, marginBottom: 15, color: "#fff" }}>Frases favoritas</Text>
      <FlatList
        data={favorites}
        keyExtractor={(_, i) => i.toString()}
        renderItem={renderItem}
        ListEmptyComponent={<Text style={{ textAlign: "center", color: "gray", marginTop: 50, fontSize: 16 }}>No tienes frases favoritas aún</Text>}
      />
    </View>
  );
}
